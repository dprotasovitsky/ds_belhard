...
def remove_outliers(df, column):
    ...
    print(f"Обнаружено выбросов: {((df[column] < lower_bound) | (df[column] >
    upper_bound)).sum()}")
...
def create_features(df):
    ...
    df.loc[:, 'Density'] = np.where(
    df['Volume'] > 0,
    df['Weight'] / df['Volume'],
    0
    )
    print("Созданы новые признаки: Volume, Length_Ratio, Height_Width_Ratio, Size_Index, Density")
    return df
...
def preprocess_data(df):
    ...
    try:
        ...
        new_size = df.shape[0]
        print(f"Удалено {original_size - new_size} выбросов ({((original_size - new_size)/original_size*100):.1f}%)")
        ...
        all_numerical = NUMERICAL_COLS + ['Volume', 'Length_Ratio', 'Height_Width_Ratio',
        'Size_Index', 'Density']
